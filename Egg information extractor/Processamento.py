...
def subImagens(elementos, img, tColumns, tLines, factor, pixFactor, dFactor, nomeArquivo):
    ...
    while proceed == False:
        ...
        # Test if egg name exists
        """if os.path.exists(egg_folder_processed_path) or os.path.exists(egg_folder_fit_plot_path):
            # mbox.showerror("Error", "File exists - Choose another file name")
            continue
        else:
            os.makedirs(egg_folder_processed_path)
            os.makedirs(egg_folder_fit_plot_path)
            proceed = True
        """
    ...
    for i in elementos:
        pos, n = i
        if n == 1:
            linhas.append(int(pos * (1 / factor)))
        else:
...
# Create and create a new directory because it does not exist
...
def subImagens(elementos, img,tColumns, tLines,factor, pixFactor, dFactor, nomeArquivo):
    ...
    while proceed == False:
        if nomeArquivo == '':
            fileName = dlg.asksaveasfilename(confirmoverwrite=False)
        else:
            (pathFolder, nameFile) = os.path.split(nomeArquivo)
            root_nameFile = os.path.splitext(nameFile)[0]
            fileName = pathFolder + f'/{root_nameFile}.dad'

        (pathFolder, nameFile) = os.path.split(fileName)
        print(pathFolder, " - ", nameFile)
        root_nameFile = os.path.splitext(nameFile)[0]
        nomeRelatorio = f'C:/ProjetoOvos/Relatorio.dad'
        arq = open(nomeRelatorio, 'at')
    ...
    for i in elementos:
        pos, n=i;
        if n==1: linhas.append(int(pos*(1/factor)))
        else: colunas.append(int(pos*(1/factor)));
    linhas.sort();
    colunas.sort();
    nFile = 1
    for x in range(len(linhas)-1):
        for y in range(len(colunas)-1):
            #while True:
            recImage = imgProcess[linhas[x]:linhas[x+1], colunas[y]:colunas[y+1]];
            ...
            nomeArquivo = pathFolder + f'/Processed/{root_nameFile}_({nFile}).jpg'
            rotateFileName = pathFolder + f'/Processed/rotate_{root_nameFile}_({nFile}).jpg'

            leituraArquivo = f'{nomeArquivo},{a},{b},{c},{d},{v},{area},{vFormulaArea},{vFormulaVolume},{pixFactor}, {dFactor}\n'
            arq.write(leituraArquivo)
            nFile+=1;

            cv2.imwrite(nomeArquivo,imgProc)
            cv2.imwrite(rotateFileName, rotateImage)
    ...
    arq.close()
    recImage = imgProcess[linhas[0]:linhas[len(linhas)-1], colunas[0]:colunas[len(colunas)-1]]
    tLines, tColumns, x = adjustImageDimension(recImage)
    down_points = (tColumns, tLines)
    dispImage = cv2.resize(recImage, down_points, interpolation=cv2.INTER_LINEAR)

    nomeArquivo = pathFolder + f'/Processed/{root_nameFile}_(Processed).jpg'
    cv2.imwrite(nomeArquivo, recImage)
...
def process(frame, factor, pixFactor):
    ...
    # Draw the line of these points on the original image
    if len(pt1) > 0 and len(pt2) > 0:
        original = cv2.line(original, (int(pt1[1]), int(pt1[0])), (int(pt2[1]), int(pt2[0])), (47, 141, 255), thickness=2)
        A = max_distance
    ...
    #Measure egg volume based on trapezoidal rule
    (volume, eggArea)=calcVolume(dicSlices, pixFactor)
    ...
    cv2.putText(original, 'D: ' + str("{:.3f}".format(D)), (original.shape[1] - 150, 90), cv2.FONT_HERSHEY_SIMPLEX,
                0.75, (255, 255, 255), 1)
    cv2.putText(original, 'V: ' + str("{:.3f}".format(volume)), (original.shape[1] - 150, 110), cv2.FONT_HERSHEY_SIMPLEX,
                0.75, (30, 105, 210), 1)

    return [original, A, B, C, D, volume, eggArea, pt1,pt2,pt3,pt4]
